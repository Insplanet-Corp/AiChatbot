...
const SEARCH_FILTER_MESSAGES = (message) => [
  ...
  { role: "assistant", content: `{"intent":"search","category":"개발","grade":null,"maxAge":null,"minAge":null}` },
  { role: "user", content: "40세 이하 고급 디자이너 추천해줘" },
  { role: "assistant", content: `{"intent":"search","category":"디자인","grade":"고급","maxAge":40,"minAge":null}` },
  { role: "user", content: "30살 이상 퍼블리셔 있을까?" },
  { role: "assistant", content: `{"intent":"search","category":"퍼블리싱","grade":null,"maxAge":null,"minAge":30}` },
  { role: "user", content: "오늘 날씨 어때요?" },
  { role: "assistant", content: `{"intent":"chat","category":null,"grade":null,"maxAge":null,"minAge":null}` },
...
const run = async () => {
  ...
  const { intent, category: categoryFilter, grade: gradeFilter, maxAge, minAge } = filters;
  ...
  console.log("\n[2] 추출된 필터");
  console.log("    등급 :", gradeFilter ?? "없음", "/ 직무 :", categoryFilter ?? "없음", "/ 나이 :", hasAgeFilter ? `${minAge ?? ""}~${maxAge ?? ""}` : "없음");
  ...
  if (gradeFilter || hasAgeFilter) console.log(`\n[5] 등급/나이 하드필터 후 : ${hardFiltered.length}건`);
